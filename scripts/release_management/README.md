...
By default, the command will try to ZIP the file at `build_${GOOS}_${GOARCH}`.
This can be changed with the `--file` input parameter.

By default, the command will output the ZIP file to `build_${CIRCLE_TAG}_${GOOS}_${GOARCH}.zip`.
...
By default, the command will use the tendermint organization/repo, which can be changed using the `--org` and `--repo` parameters.
...
By default, the command will upload the file `/tmp/workspace_${CIRCLE_TAG}_${GOOS}_${GOARCH}.zip`. This can be changed by the `--file` input parameter.
